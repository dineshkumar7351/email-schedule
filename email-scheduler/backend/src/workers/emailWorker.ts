...
// Mock Slack sending
async function sendSlackNotification(userId: string, senderEmail: string, hourWindow: string) {
  const connection = await prisma.slackConnection.findUnique({ where: { userId } });
  if (!connection) return;

  const alertKey = `slack-rate-alert:${senderEmail}:${hourWindow}`;
  const setnx = await redis.set(alertKey, '1', 'EX', 3600, 'NX');
  if (setnx) {
    try {
      logger.info(`Sending Slack notification for rate limit to user ${userId} / sender ${senderEmail}`);
      // In a real app we would call Slack API, here we log it to demonstrate functional flow
    } catch (e) {
...
async function processEmail(job: any) {
  ...
  if (currentCount > hourlyLimit) {
    ...
    if (sender) {
      await sendSlackNotification(emailRecord.userId, sender.email, hourStr);
    }
